...
class ResultsState(State):
    ...
    def display_results(self):
        clear_terminal()
        print(MaskedText(f"Words Learned: ", self.user).render())
        for word in self.new_words:
            print(MaskedText(f"- {(word + ':').ljust(12)} {len(word)} ", self.user).render())
       
        print("-"*15)
        print(MaskedText(f"Round Score: {sum(len(word) for word in self.new_words)}", self.user).render())
       
        if self.unlocks:
            print("-"*15)
            print(MaskedText(f"Unlocked: {', '.join(self.unlocks)}!", self.user).render())

        # TODO: DO SOMETHING WITH STATISTICS
        print("-"*15)
        print(MaskedText(f"Statistics: ", self.user).render())
